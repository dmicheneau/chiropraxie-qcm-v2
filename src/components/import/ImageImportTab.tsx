/**
 * ImageImportTab Component
 * Tab for importing questions from images using OCR
 * Automated flow: Select image → OCR → Detect theme → Generate questions
 */

import { useState, useRef, useEffect, useCallback } from 'react'
import { Image, Upload, AlertCircle, CheckCircle, Sparkles, Edit3 } from 'lucide-react'
import { extractTextFromImage, validateImageFile, type OCRProgress } from '@/services/import'
import {
  generateQuestions,
  detectTheme,
  AVAILABLE_THEMES,
...
export function ImageImportTab() {
  ...
  // Automated flow: Detect theme → Generate
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Start automated OCR extraction
    ...
    try {
      ...
      if (result.text.length < 50) {
        setError('Le texte extrait est trop court. Essayez une image plus lisible.')
        setWorkflowStep('idle')
        return
      }
  ...
  const getStepMessage = (): string => {
    switch (workflowStep) {
      case 'extracting':
        return ocrProgress?.message || 'Reconnaissance du texte (OCR)...'
      ...
      <div className="card bg-base-100 shadow">
        <div className="card-body">
          ...
          <p className="text-sm text-base-content/70 mb-4">
            Prenez en photo vos notes de cours. L'OCR, la détection du thème et la génération de
          ...
          {imagePreview && (
            <div className="mt-4">
              <img
                ...
                className="max-h-48 rounded-lg border border-base-300"
              />
            </div>
          )}
            ...
            <div className="mt-4 p-4 bg-base-200 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  {workflowStep === 'extracting' && ocrProgress && ocrProgress.progress > 0 && (
                    <progress
        ...
        <div className="card bg-base-100 shadow">
          <div className="card-body">
            <div className="flex items-center justify-between mb-4">
              ...
              {ocrConfidence !== null && (
                <span className={`badge ${getConfidenceColor(ocrConfidence)}`}>
                  OCR: {Math.round(ocrConfidence)}% confiance
                </span>
              )}
            </div>
            ...
            <details className="collapse collapse-arrow bg-base-200">
              ...
              <div className="collapse-content">
                <textarea
                  ...
                  placeholder="Le texte extrait apparaîtra ici. Vous pouvez le modifier avant de générer les questions."
                />
              </div>
            </details>
